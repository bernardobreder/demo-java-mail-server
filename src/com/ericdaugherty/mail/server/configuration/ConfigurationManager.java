/******************************************************************************
 * $Source$ $Revision: 167 $ $Author: edaugherty $ $Date: 2007-09-03 11:54:25
 * -0500 (Mon, 03 Sep 2007) $
 * 
 ****************************************************************************** 
...
public class ConfigurationManager implements ConfigurationParameterContants {
	...
	/**
	 * Array of domains that the SMTP server should accept mail for local
	 * delivery
	 */
	...
	public static synchronized ConfigurationManager initialize(
		...
		File userConfigFile = new File(configurationDirectory,
				userConfigFilename);
	...
	private void loadGeneralProperties() {
		...
		if (smtpServers.length() > 0) {
			...
			for (int i = 0; i < raw.length; i++) {
				...
				if (colon == -1) {
					...
					defaultSmtpServers[i].setPort(Integer.parseInt(server
							.substring(colon + 1)));
				}
				...
				if (credentials != null) {
					...
					defaultSmtpServers[i].setUsername(credentials.substring(0,
							colon));
					defaultSmtpServers[i].setPassword(credentials
							.substring(colon + 1));
				}
			}
		} else {
		...
		String defaultUserString = properties.getProperty(DEFAULT_USER, "")
				.trim();
		;
	...
	private void loadUserProperties() {
		...
		while (propertyKeys.hasMoreElements()) {
			...
			if (key.startsWith(USER_DEF_PREFIX)) {
				...
				try {
					users.put(correctedUsername,
							loadUser(fullUsername, properties));
				} catch (InvalidAddressException e) {
					log.warn("Skipping user: " + fullUsername
							+ ".  Address is invalid.");
				}
			}
		}
