 ...
 * name contains a map indexed by type.
 *
...
class NameSet {
  ...
  /**
   * Creates an empty NameSet for use as a Zone or Cache. The origin is set to
   * the root.
   */
  ...
  protected Object lookup(Name name, int type) {
    ...
    for (tlabels = olabels; tlabels <= labels; tlabels++) {
      ...
      /*
       * If this is the name and this is a cache, look for an NXDOMAIN entry.
       */
      if (isexact && isCache) {
  ...
  /**
   * Removes the given set with the name and type. The data contained in the set
   * is abstract.
   */
