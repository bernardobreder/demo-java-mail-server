...
public class Cache extends NameSet {
  ...
  public SetResponse lookupRecords(Name name, int type, int minCred) {
    ...
    for (int i = 0; i < objects.length; i++) {
      ...
      if (rrset == null) {
        ...
        /*
         * If we're not looking for type ANY, return NXRRSET. Otherwise ignore
         * this.
         */
        if (type != Type.ANY) {
          if (verbose)
    ...
    /*
     * As far as I can tell, the only legitimate time cr will be null is if we
   ...
   * implementation is found in org.xbill.DNSSEC.security.DNSSECVerifier, which
   * requires Java 2 or above and the Java Cryptography Extensions.
   */
  ...
  /**
   * Mandates that all data stored in this Cache must be verified and proven to
   * be secure, using a verifier (as defined in setVerifier).
   */
