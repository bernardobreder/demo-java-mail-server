/******************************************************************************
 * $Workfile: DeliveryService.java $ $Revision: 164 $ $Author: edaugherty $
...
public class DeliveryService implements ConfigurationParameterContants {
	...
	private Hashtable<String, Date> authenticatedIps;

	/** The mailboxes that are currently locked */
	private Hashtable<String, String> lockedMailboxes;
	...
	protected DeliveryService() {
		...
		authenticatedIps = new Hashtable<String, Date>();
		// Initialize the Hashtable for tracking locked mailboxes
		lockedMailboxes = new Hashtable<String, String>();
	}
	...
	public boolean acceptAddress(EmailAddress address, String clientIp,
		...
		isValid = isLocalAddress(address) || // Accept all local email.
				...
				|| (isRelayApproved(clientIp,
						configurationManager.getRelayApprovedIpAddresses()) || (isRelayApprovedForEmail(
						clientFromAddress,
	...
	private boolean isAuthenticated(String clientIp) {
		...
		if (authenticatedIps.containsKey(clientIp)) {
			...
			// Calculate the current time and the time that the login will
			// timeout.
			long currentTime = System.currentTimeMillis();
			long timeoutTime = authenticationDate.getTime()
		...
		for (int index = 0; index < approvedAddresses.length; index++) {
			...
			// Check for a partial match
				...
				if (wildcardIndex != -1) {
					...
					StringTokenizer clientIpTokenizer = new StringTokenizer(
							clientIp, ".");
					...
					while (clientIpTokenizer.hasMoreTokens()) {
						try {
							clientIpToken = clientIpTokenizer.nextToken()
									.trim();
							approvedAddressToken = approvedAddressTokenizer
									.nextToken().trim();
							...
							log.warn("Invalid ApprovedAddress found: "
									+ approvedAddress + ".  Skipping.");
							isMatch = false;
							break;
						}
					}
	 ...
	 * 
	 * @param clientFromEmail
	 *            The email address to test.
	 * @param approvedEmailAddresses
	 *            The approved list.
	...
	private boolean isRelayApprovedForEmail(EmailAddress clientFromEmail,
		...
		for (int index = 0; index < approvedEmailAddresses.length; index++) {
			...
			if (clientFromEmail.getAddress().equalsIgnoreCase(
