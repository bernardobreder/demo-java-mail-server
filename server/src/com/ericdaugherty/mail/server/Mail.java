/******************************************************************************
 * $Workfile: Mail.java $ $Revision: 120 $ $Author: edaugherty $ $Date:
 * 2003-10-15 14:06:23 -0500 (Wed, 15 Oct 2003) $
 * 
 ****************************************************************************** 
...
public class Mail {
	...
	/**
	 * Logger Category for this class. This variable is initialized once the
	 * main logging system has been initialized
	 */
	...
	/**
	 * Provides a 'safe' way for the application to shut down. This method is
	 * provided to enable compatability with the JavaService NT Service wrapper
	 * class. It defers the call to the shutdown method.
	 * 
	 * @param args
	 */
	 ...
	 * to stop the running threads. If the threads to not stop within 60
	 * seconds, the application will force the threads to stop by calling
	 * System.exit();
	 */
	...
	/**
	 * This method is the entrypoint to the system and is responsible for the
	 * initial configuration of the application and the creation of all
	 * 'service' threads.
	 */
	public static void main(String[] args) {
		...
		try {
			...
			// Start the SMTPSender thread (This thread actually delivers the
			// mail recieved
			// by the SMTP threads.
	...
	private static String getConfigurationDirectory(String[] args) {
		...
		// Otherwise, use the default, which is 'mail.conf' in the current
		// directory.
		else if ((directoryFile = new File(directory)).exists()) {
			System.out
					.println("Configuration Directory not specified.  Using \""
							+ directoryFile.getAbsolutePath() + "\"");
		}
		// If no file was specified and the default does not exist, printing out
		// a usage line.
		else {
			System.out
					.println("Usage:  java com.ericdaugherty.mail.server.Mail <configuration directory>");
	...
	/**
	 * This method is responsible for initialzing the logging mechanism used by
	 * this application. All required information is loaded from the
	 * configuration file.
	 */
	private static void initializeLogging(String configurationDirectory) {
		...
		if (!logConfigurationFile.exists()) {
			...
			try {
				...
				// String filename )
				// method.
				Class<?> propertyConfigurator = Class
				...
				configureMethod
						.invoke(null, new Object[] { logConfigurationFile
								.getAbsolutePath() });
				...
				// log4j is not available.
				System.err
						.println("Found log4j Class but method configureAndWatch(String) is not available.");
			} catch (IllegalAccessException illegalAccessException) {
				// log4j is not available.
				System.err
						.println("Found log4j Class but method configureAndWatch(String) caused an IllegalAccessException.");
	...
	/**
	 * Initializes the logger with the default configuration if no log file is
	 * found.
	 * 
	 * @param logConfigurationFile
	 *            the file that may contain properties to use to configure the
	 *            default logger.
	 */
	private static void initializeDefaultLogging(File logConfigurationFile) {
		...
		if (logConfigurationFile.exists()) {
			...
			try {
				...
				threshold = logConfigurationProperties
						.getProperty(
				...
				if (!threshold.equals("debug") && !threshold.equals("info")
						...
						&& !threshold.equals("error")
						&& !threshold.equals("fatal")) {
					System.err
							.println("Invalid value for property "
									+ ConfigurationParameterContants.LOGGING_DEFAULT_THRESHOLD
									+ ": " + threshold);
